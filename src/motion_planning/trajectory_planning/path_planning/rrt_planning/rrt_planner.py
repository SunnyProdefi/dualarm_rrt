...
from mpl_toolkits.mplot3d import Axes3D  # 从mpl_toolkits.mplot3d导入Axes3D，用于创建3D图形
...
class RRTPlanner:
    def __init__(self, rrt_map: RRTMap, rrt_parameter: RRTParameter, pool: multiprocessing.Pool = None) -> None:
        ...
        self._check_collision = rrt_parameter.create_check_collision(rrt_map)  # 创建碰撞检测函数
    ...
    def plan(self, pool: multiprocessing.Pool = None) -> None:
        ...
        if self.animation:
            self.draw_graph(self.goal, self._path, True)  # 如果开启动画，最后绘制完整路径图
        return

    def sample(self) -> Node:
        # 随机采样节点函数
        if random.randint(0, 100) > self.goal_sample_rate:
            rnd = [random.uniform(*self._area[i]) for i in range(len(self._area))]  # 以一定概率随机采样点
        else:
    ...
    def get_nearest_list_index(self, rnd: Node) -> int:
        # 获取最近节点的索引
        d_list = [Distance.point_to_point(node.point, rnd.point) for node in self.nodes]  # 计算所有节点到随机点的距离
    ...
    def get_new_node(self, n_ind: int, rnd: Node) -> Node:
        ...
        unit_vector = UnitVector(nearest_node.get_point(), rnd.get_point())  # 计算单位向量
        new_point = nearest_node.get_point() + self._expand_dis * unit_vector  # 沿单位向量扩展一定距离
        new_node = Node(new_point, cost=nearest_node.cost + self._expand_dis, parent=n_ind)  # 创建新节点
        return new_node  # 返回新节点

    def is_near_goal(self, pool: multiprocessing.Pool = None) -> bool:
        # 检查是否靠近目标
        d = Distance.point_to_point(self.nodes[-1].get_point(), self.goal.get_point())  # 计算最新节点到目标的距离
